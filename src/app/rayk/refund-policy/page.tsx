...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const page = settings.rayk.legalPages["refund-policy"];
  return pageMetadata({
    title: page.metaTitle,
    description: page.metaDescription,
    canonical: raykAbsoluteUrl("/rayk/refund-policy"), siteName: "RAYK",
  });
}

export default async function RaykRefundPolicyPage() {
  const settings = await readSiteSettings();
  const page = settings.rayk.legalPages["refund-policy"];
