    ...
    <div className="max-w-2xl mx-auto px-4 py-16">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 mb-10">
        ...
        <button type="submit"
          ...
          className="border border-black px-6 py-3 text-xs font-semibold tracking-widest uppercase hover:bg-black hover:text-white transition-colors disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-black"
        >
      ...
      {loading && (
        <div className="border border-black/10 py-10 text-center anim-fade-slide-in">
          <svg className="w-7 h-7 mx-auto mb-2 animate-spin text-black/40" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
          <p className="text-[10px] font-semibold tracking-[0.3em] uppercase text-black/40">Looking up order…</p>
        </div>
      )}

      {error && !loading && (
        <div className="border border-black/10 py-10 px-6 text-center anim-fade-slide-in">
          <svg className="w-9 h-9 mx-auto mb-3 text-black/20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-xs font-semibold tracking-[0.3em] uppercase text-black mb-1.5">No order found</p>
          <p className="text-xs text-black/50 tracking-wide">{error}</p>
        </div>
      )}

      {!loading && !error && !order && !ref && (
        <div className="border border-black/10 py-10 px-6 text-center">
          <svg className="w-9 h-9 mx-auto mb-3 text-black/20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-xs font-semibold tracking-[0.3em] uppercase text-black mb-1.5">Enter your reference</p>
          <p className="text-xs text-black/50 tracking-wide">You received it in your order confirmation message.</p>
        </div>
      )}
