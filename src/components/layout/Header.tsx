...
export default function Header() {
  ...
  const [instagramUrl, setInstagramUrl] = useState(DEFAULT_SITE_SETTINGS.contact.instagramUrl);
  const [instagramHandle, setInstagramHandle] = useState(DEFAULT_SITE_SETTINGS.contact.instagramHandle);
    ...
    fetch("/api/storefront/site-settings")
      ...
      .then((d) => {
        if (d.success && d.data?.contact) {
          if (d.data.contact.instagramUrl) setInstagramUrl(d.data.contact.instagramUrl);
          if (d.data.contact.instagramHandle) setInstagramHandle(d.data.contact.instagramHandle);
        }
      })
      .catch(() => {});
  }, []);
